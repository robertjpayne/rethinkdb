...
private:
    static void maybe_generate_key(counted_t<table_t> tbl,
                                   ...
                                   vector_t<std::string> *generated_keys_out,
    ...
    eval_impl(scope_env_t *env, args_t *args, eval_flags_t) const {
        ...
        if (!done) {
            ...
            for (;;) {
                ...
                vector_t<bool> pkey_was_autogenerated(datums.size());
...
private:
    virtual scoped_ptr_t<val_t> eval_impl(
        ...
        if (v0->get_type().is_convertible(val_t::type_t::SINGLE_SELECTION)) {
            ...
            batchspec_t batchspec = batchspec_t::user(batch_type_t::TERMINAL, env->env);
            for (;;) {
                vector_t<datum_t> vals = ds->next_batch(env->env, batchspec);
                if (vals.empty()) {
                    break;
                }

                scoped_ptr_t<vector_t<datum_t> > keys;
                if (f->is_deterministic() != deterministic_t::always) {
                    keys = make_scoped<vector_t<datum_t> >();
