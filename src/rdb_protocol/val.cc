...
public:
    ...
    virtual datum_t replace(
        ...
        return_changes_t return_changes,
        ignore_write_hook_t ignore_write_hook) {
        vector_t<datum_t > vals{get()};
        vector_t<datum_t > keys{
            vals[0].get_field(
...
datum_t table_t::batched_replace(
    env_t *env,
    const vector_t<datum_t> &vals,
    const vector_t<datum_t> &keys,
    ...
    if (replacement_generator->is_deterministic() != deterministic_t::always) {
        ...
        vector_t<datum_t> replacement_values;
        ...
        vector_t<bool> pkey_was_autogenerated(vals.size(), false);
...
datum_t table_t::batched_insert(
    env_t *env,
    vector_t<datum_t> &&insert_datums,
    vector_t<bool> &&pkey_was_autogenerated,
    ...
    datum_object_builder_t stats;
    vector_t<datum_t> valid_inserts;
    vector_t<bool> valid_pkey_was_autogenerated;
    vector_t<datum_t> insert_keys;
