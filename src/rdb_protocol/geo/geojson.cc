...
ql::datum_t construct_geo_latlngrect(
    ...
    coordinates.push_back(datum_t(rect.lng().lo()));
    coordinates.push_back(datum_t(rect.lng().hi()));
...
scoped_ptr_t<S2LatLngRect> coordinates_to_s2latlngrect(const datum_t &coords) {
    ...
    The format for `coords` is an array with the R1Interval and the S1Interval
    coordinates. */
    if (coords.arr_size() != 4) {
        throw geo_exception_t(
            "LatLngRect must have four angles in its coordinates field.");
    }

    double lat_lo = coords.get(0).as_num();
    double lat_hi = coords.get(1).as_num();
    if (fabs(lat_lo) > M_PI_2 || fabs(lat_hi) > M_PI_2) {
        throw geo_exception_t("Latitude out of range for LatLngRect.");
    }
    geo::R1Interval lat(lat_lo, lat_hi);

    double lng_lo = coords.get(2).as_num();
    double lng_hi = coords.get(3).as_num();
    if (fabs(lng_lo) > M_PI || fabs(lng_hi) > M_PI) {
        throw geo_exception_t("Longitude out of range for LatLngRect.");
    }
    geo::S1Interval lng;
    // The full S1Interval requires special handling because the public `S1Interval`
    // constructor will otherwise wrap around the lower bound and the interval will
    // become the single longitude [M_PI, M_PI].
    if (lng_lo == -M_PI && lng_hi == M_PI) {
        lng = geo::S1Interval::Full();
    } else {
        lng = geo::S1Interval(lng_lo, lng_hi);
    }

    scoped_ptr_t<S2LatLngRect> result(new S2LatLngRect(lat, lng));
    if(!result->is_valid()) {
...
void validate_geojson(const ql::datum_t &geojson) {
    ...
    public:
        ...
        void on_latlngrect(UNUSED const S2LatLngRect &) {
            throw geo_exception_t("LatLngRect cannot be generated by a user query.  "
                                  "The `$reql_LatLngRect$` type is reserved for "
                                  "internal use.");
        }
    };
