...
void S2RegionCoverer::FloodFill(
    S2Region const& region, S2CellId const& start, vector_t<S2CellId>* output) {
  unordered_set<S2CellId, std::hash<S2CellId> > all;
  vector_t<S2CellId> frontier;
  output->clear();
  all.insert(start);
