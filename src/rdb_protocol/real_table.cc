...
template<class T>
vector_t<vector_t<T> > split(vector_t<T> &&v) {
    vector_t<vector_t<T> > out;
...
optional<counted_t<const ql::func_t> > real_table_t::get_write_hook(
    ql::env_t *env,
...
ql::datum_t real_table_t::write_batched_replace(
    ql::env_t *env,
    const vector_t<ql::datum_t> &keys,
    ...
    vector_t<store_key_t> store_keys;
    ...
    vector_t<vector_t<store_key_t> > batches = split(std::move(store_keys));
...
ql::datum_t real_table_t::write_batched_insert(
    ...
    UNUSED vector_t<bool> &&pkey_is_autogenerated,
    ...
    ql::datum_t stats((std::map<datum_string_t, ql::datum_t>()));
    std::set<std::string> conditions;
    vector_t<vector_t<ql::datum_t> > batches = split(std::move(inserts));
    for (auto &&batch : batches) {
